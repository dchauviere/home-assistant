...
class HomeAssistantTydomData(NamedTuple):
...
async def async_setup_entry(
    ...
    hass_data: HomeAssistantTydomData = hass.data[DOMAIN][entry.entry_id]

    @callback
    def async_discover_device(device_ids: list[str]) -> None:
        """Discover and add a discovered tuya cover."""
        entities: list[TydomCoverEntity] = []
        for device_id in device_ids:
            device = hass_data.device_manager.device_map[device_id]
            entities.append(
                TydomCoverEntity(device, hass_data.device_manager, "description")
            )
    ...
    entry.async_on_unload(
        async_dispatcher_connect(hass, "tydom_discovery_new", async_discover_device)
    )


class TydomCoverEntity(TuyaEntity, CoverEntity):
    """Tuya Cover Device."""
